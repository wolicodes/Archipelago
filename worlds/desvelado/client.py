...
from NetUtils import NetworkItem, NetworkPlayer, decode, encode

DEBUG = False


class DesveladoCommandProcessor(ClientCommandProcessor):
    def _cmd_state(self):
        """Check Desvelado Connection State"""
        if isinstance(self.ctx, DesveladoContext):
            logger.info(f"Desvelado Status: {self.ctx.get_desvelado_status()}")


class DesveladoContext(CommonContext):
    game = "Desvelado"
    items_handling = 0b111
    command_processor = DesveladoCommandProcessor
    ...
    def get_desvelado_status(self) -> str:
        if not self.is_proxy_connected():
            return "Not connected to Desvelado"
        return "Connected to Desvelado"

    async def send_msgs_proxy(self, msgs: str) -> bool:
    ...
    def is_connected(self) -> Endpoint | None | bool:
    ...
    def on_print_json(self, args: dict):
        text = self.rawjsontotextparser(deepcopy(args["data"]))
    ...
    def make_gui(self):
        ...
        class TextManager(GameManager):
            base_title = "Desvelado Client v1.0.0"

        return TextManager


async def proxy(websocket, path: str = "/", ctx: DesveladoContext | None = None):
...
async def on_client_connected(ctx: DesveladoContext):
...
async def proxy_loop(ctx: DesveladoContext):
    try:
        ...
        logger.exception(e)
        logger.info("Aborting Desvelado Proxy Client due to errors")


def launch_desvelado_client(*launch_args: str):
    async def main():
        parser = get_base_parser()
        args = parser.parse_args(launch_args)

        ctx = DesveladoContext(args.connect, args.password)
        logger.info("Starting Desvelado proxy server")
    ...
    Utils.init_logging("DesveladoClient")
